...
import {
  careerListingMapper,
  careerSingleMapper,
  categoryListingMapper,
  categorySingleMapper,
} from "src/users/user.mapper";
import {
  listingApiWrapper,
  errorApiWrapper,
  ...
  async findAllCareer(
    ...
    [careers, total] = await this.findAndCount({
      ...
      where: (qb) => {
        ...
        if (categoryId) {
          qb.innerJoin("Career.category", "category").andWhere(
      ...
      //     ...(categoryId && {
      //       careerBycategory: { category_id: 60 },
      //     }),
      //   },
      order: {
        ...(sort_by && {
        ...
        ...(Boolean(years_needed) && {
          years_needed: years_needed,
        }),
        ...(atar &&
          Boolean(CareerSortType[atar]) && {
            admission_rank: CareerSortType[atar],
          }),
        ...(Boolean(course_cost) &&
          course_cost && {
            cost_course: course_cost,
          }),
        ...(Boolean(status_in_company) &&
          status_in_company && {
            status_in_company: status_in_company,
          }),
        ...(Boolean(scope_of_skill) &&
          scope_of_skill && {
            scope_of_skill: scope_of_skill,
          }),
        ...(Boolean(job_help_environment) &&
          job_help_environment && {
            job_help_environment: job_help_environment,
          }),
        ...(Boolean(job_help_people) &&
          job_help_people && {
            job_help_people: job_help_people,
          }),
        ...(work_life_balance &&
          Boolean(work_life_balance) && {
    ...
    if (careers.length > 0) {
      ...
      for await (const iterator of careers) {
        ...
        Object.assign(iterator, {
          userLike: await this.checkIfuserLike(user_id, iterator?.id),
        });
      }
    }
  ...
  async updateCount(id: number, updateCountDto: UpdateCountDto, user: User) {
    try {
      ...
      if (!checkIfAlreadLiked) {
        ...
        await getRepository(CareerLike)
          .createQueryBuilder()
          .softDelete()
          .where({
            user_id: user?.id,
